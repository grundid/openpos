...
public class JPanelCloseMoney extends JPanel implements JPanelView, BeanFactoryApp {
	...
	public void activate() throws BasicException {
		loadData(PaymentsModel.loadInstance(m_App));
	}
	...
	private void loadData(PaymentsModel paymentsModel) throws BasicException {
		// Reset
		...
		// LoadData
		m_PaymentsToClose = paymentsModel;
		// Populate Data
	...
	private void printPayments(String report) {
		...
		if (sresource == null) {
			...
			try {
				...
				IReportsPublishService reportsPublishService = OpenPos.getApplicationContext().getBean(
						IReportsPublishService.class);
	...
	private void initComponents() {
		...
		cashReportButton.setText(AppLocal.getIntString("Button.CashReport"));
						...
						jPanel1Layout
								...
								.addGroup(
										jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
												.addComponent(cashReportButton).addComponent(m_jCloseCash)
												.addComponent(m_jPrintCash)).addContainerGap()));
		add(jPanel1, java.awt.BorderLayout.CENTER);
	}// </editor-fold>//GEN-END:initComponents

	protected void cashReport(ActionEvent evt) {
		try {
			ICashFromToDialog cashFromToDialog = OpenPos.getApplicationContext().getBean(ICashFromToDialog.class);
			CashFromToModel cashFromToModel = cashFromToDialog.getCashFromToModel();
			if (cashFromToModel != null)
				loadData(PaymentsModel.loadInstance(m_App, cashFromToModel.getFromDate(), cashFromToModel.getToDate()));
		}
		catch (Exception e) {
			...
			try {
				...
				if (m_App.getActiveCashDateEnd() == null) {
					new StaticSentence(m_App.getSession(),
							"UPDATE CLOSEDCASH SET DATEEND = ? WHERE HOST = ? AND MONEY = ?", new SerializerWriteBasic(
									new Datas[] { Datas.TIMESTAMP, Datas.STRING, Datas.STRING })).exec(new Object[] {
							dNow, m_App.getProperties().getHost(), m_App.getActiveCashIndex() });
				}
			}
